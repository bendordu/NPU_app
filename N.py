...
#from battery_check import Battery            -----Закомментировала для тестов на Windows------
from threading import Thread


class Window(QWidget): 
    '''Основа всех окон, отрисовка основных элементов, сетки'''
    ...
    def __init__(self):
        ...
        # self.grid.addWidget(self.bat, 2, 4)
...
class CSV(QtCore.QObject):

    def write_csv(self): 
        
        path = "doc/results/" + str(date_global) + '_' + str(patient_global) + '_init' + ".csv"
        with open(path, "w", newline='', encoding='utf-8') as csv_file:
            writer = csv.writer(csv_file)
            writer.writerow([str(date_global), str(patient_global)])
            writer.writerow(['time', 'EMG', 'GSR(Om)', 'HR'])
            writer.writerows(list(zip(time_mark, emg_data, gsr_data)))
...
class Detect(QtCore.QObject):
    ...
    def run(self):
        ...
        # while True:            
        #     emg_data.append(grove_emg_detector.main())
        #     gsr_data += [grove_gsr_sensor.main()]
            # time_mark += [time_m.elapsed()]
...
class Init(Window):
    '''9 предварительных старт-проб белым шумом'''
    ...
    def open_w(self):
        ...
        self.inst = QSound("stimuls/inst.wav")
        self.sound = QSound("stimuls/whitenoise.wav")
        self.timer = QtCore.QTimer()

        self.inst.play()
        QtTest.QTest.qWait(67000)  

        self.thread = QtCore.QThread()
        self.detect = Detect()
        self.detect.moveToThread(self.thread)
        self.thread.started.connect(self.start_init)
...
class Check(Window):
    ...
    def open_w(self):
        ...
        '''Переход на вступительную часть теста - 9 старт-проб для привыкания'''
    def start(self):
        self.close()
        self.pat = Init()
        self.pat.show()  
...
# class Check_detect(Window):

# 	def open_w(self):

# 		gsr_label = QLabel('GSR', self) 
#         self.grid.addWidget(date, 1, 0)

#         gsr = QLabel(grove_gsr_sensor.main(), self) 
#         self.grid.addWidget(date, 1, 2)

#         patient = QLabel('EMG', self) 
#         self.grid.addWidget(patient, 2, 0)

#         patient = QLabel(grove_gsr_sensor.main(), self) 
#         self.grid.addWidget(patient, 2, 2)

#         hr = QLabel('HR', self) 
#         self.grid.addWidget(sens, 3, 0)

#         hr = QLabel('HR', self) 
#         self.grid.addWidget(sens, 3, 2)


class Calendar(Window):
    '''Календарь. Выбор даты. 1 шаг'''
    ...
    def showDate(self, date):
        ...
        if ret == QMessageBox.Yes:
            global date_global
            date_global = date.toString(QtCore.Qt.ISODate)
            self.next_to_patient()
        

        '''Список пациентов. Выбор пациента или его добавление. Шаг 2'''
...
class Script_test(Script):
    ...
    def item_choose(self, item):
        ...
        '''Переход к проверке выбранных значений. Шаг 5'''
...
class Patients_test(Patients):
    ...
    def show_pat(self, item):
        ...
        '''Переход на окно определения чувствительности кэлектростимуляции. Выбор значений. Шаг 3'''
...
class AddPat(Window):
    '''Добавление новых пациентов в список''' #клавиатура пока не работает, нужно доделать
...
class Sens_main(SensTest):
    '''_main - переход с главной страницы'''
...
class Sens_test(SensTest):
    '''_test - окно, отображающееся перед началом теста'''
    ...
    def show_amp(self):
        ...
        '''Переход выбор сценария теста. Шаг 4'''
...
class NPU(Window):
    ...
    def start_test(self):
        self.close()
        self.start = Calendar() #в одну строчку не записать - не работает, поэтому сокращать можно даже не пытаться, но можно попытаться объединить в одну функцию
        self.start.show()
